...
export function Grafico({ data }: { data: GraficoIndicadorSucursalI[] }) {
  const [estadoTk, setEstadoTk] = useState<boolean>(true);
  const [estadoTkPromedio, setEstadoTkPromedio] = useState<boolean>(true);
  const [estadoMonto, setEstadoMonto] = useState<boolean>(true);
  const [estadoPrecioPromedio, setEstadoPrecioPromedio] =
    useState<boolean>(true);
  const [estadoCantidad, setEstadoCantidad] = useState<boolean>(true);
    ...
    <Stack sx={{ width: "100%" }}>
      <Stack direction="row">
        ...
        <FormControlLabel
          ...
          control={
            <Checkbox
              onChange={(event) => setReverseLeft(event.target.checked)}
            />
          }
        ...
        <FormControlLabel
          ...
          control={
            <Checkbox
              onChange={(event) => setReverseRight(event.target.checked)}
            />
          }
              ...
              onClick={(e) => {
                if (e.dataKey) {
                  ...
                  if (e.dataKey === "ticketPromedio") {
                    setEstadoTkPromedio((prev)=> !prev);
                  }
                  if (e.dataKey === "tickets") {
                    setEstadoTk((prev)=> !prev);
                  }
                   if (e.dataKey === "cantidad") {
                    setEstadoCantidad((prev)=> !prev);
                  }
                    if (e.dataKey === "ventaDiaria") {
                    setEstadoMonto((prev)=> !prev);
                  }
                }
              }}
            />
