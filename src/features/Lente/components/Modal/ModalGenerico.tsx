...
import type { InformacionAsesor, Ocupacional } from "../../interface/asesor.interface";
import CloseIcon from "@mui/icons-material/Close";


interface InformacionProps {
  data: InformacionAsesor;
  onClose: () => void;
}
...
export const ModalGenerico = ({ data, onClose }: InformacionProps) => {
  ...
  const antireflejo = data.antireflejo[0].tratamiento.map((item) => ({
    _id: item.tratamiento,
    cantidad: item.cantidad,
    porcentaje: item.porcentaje,
  }));


  const calcularTotal = (arr: Ocupacional[]) => arr.reduce((total, item) => total + item.cantidad, 0);
          ...
          <Table size="small">
            <TableHead>
              <TableRow sx={{ backgroundColor: '#e0f2f1', color: '#00786a' }}>
                <TableCell sx={{ fontWeight: 'bold', color: '#00786a' }}>{idKey === "_id" ? "Tratamiento" : "Tipo"}</TableCell>
  ...
  return (
    <Dialog open onClose={onClose} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers sx={{ p: 3 }}>
        ...
        {renderTable("Antireflejos", antireflejo, "_id")}

        {renderTable("Progresivos", progresivos, "_id")}

        {renderTable("Ocupacional", ocupacional, "_id")}
